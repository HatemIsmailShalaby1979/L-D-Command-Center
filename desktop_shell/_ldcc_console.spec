...
shutil.copy2(ROOT / "desktop_shell" / "app.py",
...
a = Analysis(
    ...
    hiddenimports=sorted(set(hiddenimports)) + [
        # The build host's Python installation does not expose Tk cleanly to
        # PyInstaller's automatic hook, so keep the GUI runtime explicit.
        "tkinter", "tkinter.ttk", "tkinter.messagebox",
        "tkinter.simpledialog", "_tkinter",
        # Project E.T. voice stack (2026-09-06): lazy imports need
        # explicit declaration — sounddevice wraps PortAudio via cffi,
        # faster-whisper carries ctranslate2; both load lazily at
        # runtime so static analysis misses them.
        "sounddevice", "soundfile", "numpy", "ctranslate2",
        "faster_whisper",
        # Desktop shell modules that are dynamically imported
        "desktop_shell.controller",
        "desktop_shell.et_ui",
        "desktop_shell.lab_ui",
        "desktop_shell.exams_ui",
        "desktop_shell.skills_ui",
        "desktop_shell.et_ui",
    ],
    hookspath=[],
    runtime_hooks=[str(ROOT / "desktop_shell" / "tk_runtime_hook.py")],
    excludes=[
        "piper", "piper_tts",  # optional TTS backend
        "torch", "torchvision", "torchaudio", "transformers",
        "matplotlib", "scipy", "sympy",  # unused optional ML stacks
        "pytest", "pytest_cov",  # test-only dependencies
    ],
)
...
if sys.platform == "win32":
    _win_tcl = Path(_base) / "tcl"
    _win_dlls = Path(_base) / "DLLs"
    for dll in ("tcl86t.dll", "tk86t.dll"):
        candidate = _win_dlls / dll
        if candidate.exists():
            _tk_binaries.append((dll, str(candidate)))
    for tree_name in ("tcl8.6", "tk8.6"):
        tree = _win_tcl / tree_name
        if tree.is_dir():
            for f in tree.rglob("*"):
                if f.is_file():
                    _tk_datas.append((str(Path("tcl") / tree_name / f.relative_to(tree)), str(f)))
if _libdir.is_dir():
...
# Some Windows Python installations make PyInstaller's tkinter hook report
# "installation is broken" even though the package files are present. Copy
# the pure-Python package explicitly so `import tkinter` works in the frozen
# app, and include the extension module beside it.
_tk_pkg = Path(_base) / "Lib" / "tkinter"
if _tk_pkg.is_dir():
    for f in _tk_pkg.rglob("*.py"):
        a.datas.append((str(Path("tkinter") / f.relative_to(_tk_pkg)), str(f), "DATA"))
_tk_extension = Path(_base) / "DLLs" / "_tkinter.pyd"
if _tk_extension.exists():
    a.binaries.append(("_tkinter.pyd", str(_tk_extension), "EXTENSION"))
