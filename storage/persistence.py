...
ALLOWED_KINDS = {
    ...
    "capabilities", "verdicts", "linkedin_posts", "interview_preps",
    "et_sessions", "level_exams", "placement",
}
...
class Storage:
    ...
    def set_preference(self, key: str, value: Any) -> None:
        ...
        * ``"job_watch"`` — Career agent watchlist config (dict).
          ...
          whether to run search_jobs_now(). Empty company lists fall
          back to the shared ``job_sources`` preference, then to the
          engine defaults (see engines/career-engine/job_boards.py).

        * ``"job_sources"`` — Saved company rosters for the job boards
          (dict). Schema::

              {
                "greenhouse_companies": [str],  # board tokens
                "lever_companies": [str],
                "ashby_companies": [str],
              }

          Written via controller.save_job_sources(); read by
          _job_sources() which falls back to the engine's
          contact-center/CX defaults when the key is absent.

        * ``"career_identity"`` — The career memory (2026-09-05): every
          fact the app has learned about the user's accounts. Schema::

              {
                "github_username": str,
                "github_projects": [{"name","description","tech"}],
                "github_linked_at": "<ISO timestamp>",
                "linkedin": {"name": str, "email": str, "sub": str,
                             "linked_at": "<ISO timestamp>"},
                "target_role": str,
                "target_role_at": "<ISO timestamp>",
              }

          Written by import_github_projects / fetch_linkedin_profile /
          enhance_resume; read by get_career_identity (offline cache,
          restart-surviving). Cleared only by the user (clear_career_identity).
        """
        prefs = {}
