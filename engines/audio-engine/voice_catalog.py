...
# Mr. & Mrs. E.T. voice pairs (2026-09-06, Project E.T.): (male, female)
# per language. Where no real second voice exists in the Piper catalog,
# the pair is absent and the conversation engine differentiates the
# personas with assembly.pitch_shift on the single voice — honest
# fallback, never a failed conversation.
# Candidate ids will be HEAD-probe-verified by provisioning on first
# use; unresolvable ids fall back to pitch-shift automatically.
PIPER_LANGUAGE_VOICE_PAIRS: dict[str, tuple[str, str]] = {
    "en": ("en_US-ryan-medium", "en_US-amy-medium"),   # both in speaker pool
    "de": ("de_DE-thorsten-medium", "de_DE-ramona-low"),
    "es": ("es_ES-mls_10246-low", "es_ES-mls_male_19946-low"),
    ...
    "ru": ("ru_RU-dmitri-medium", "ru_RU-denis-medium"),
...
def et_voice_pair(language: str) -> tuple[str, str, bool]:
    """Mr. & Mrs. E.T. voices for one language: (male, female, real_pair).

    real_pair=True means both ids are distinct concrete Piper voices.
    real_pair=False means the language has one voice — the female
    persona must be pitch-shifted from it by the caller (assembly
    .pitch_shift); the engine announces nothing, it just works."""
    lang = (language or FALLBACK_LANGUAGE).lower()[:2]
    if lang in PIPER_LANGUAGE_VOICE_PAIRS:
        male, female = PIPER_LANGUAGE_VOICE_PAIRS[lang]
        return male, female, male != female
    voice = _resolve_language(lang, PIPER_LANGUAGE_VOICES, "E.T. pair")
    return voice, voice, False
