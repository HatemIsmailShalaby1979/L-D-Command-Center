...
# ---------------------------------------------------------------------------
# Pitch shift (Project E.T., 2026-09-06)
# ---------------------------------------------------------------------------

def pitch_shift(wav_bytes: bytes, semitones: float = -3.0) -> bytes:
    """Contract: shift a WAV's pitch by `semitones` semitones without
    changing perceived duration materially. Resample-based (linear
    interpolation): a few cents of quality loss for a big win — one
    voice can serve both E.T. personas when a language has no real
    female/male pair (voice_catalog.et_voice_pair real_pair=False).

    Negative semitones = deeper (Mr. E.T. from a female-leaning base),
    positive = brighter (Mrs. E.T.). Clamped to ±5 semitones: beyond
    that, chipmunk/rumble territory.
    """
    semitones = max(-5.0, min(5.0, float(semitones)))
    if abs(semitones) < 0.05:
        return wav_bytes
    pcm, rate = _parse_wav_to_pcm(wav_bytes)
    samples = struct.unpack(f"<{len(pcm)//2}h", pcm)
    ratio = 2.0 ** (semitones / 12.0)
    out_len = max(1, int(len(samples) / ratio))
    shifted = bytearray(out_len * 2)
    for i in range(out_len):
        pos = i * ratio
        base = int(pos)
        frac = pos - base
        if base + 1 < len(samples):
            value = (samples[base] * (1 - frac)
                     + samples[base + 1] * frac)
        else:
            value = samples[min(base, len(samples) - 1)]
        clamped = int(max(-32768, min(32767, value)))
        struct.pack_into("<h", shifted, i * 2, clamped)
    return _make_wav_header(len(shifted), rate) + bytes(shifted)
