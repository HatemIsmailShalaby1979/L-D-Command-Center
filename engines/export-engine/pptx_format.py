...
def export_journey_to_pptx(journey: dict[str, Any]) -> bytes:
    ...
    buf = BytesIO()
    prs.save(buf)
    # python-pptx stamps ZIP members with the wall clock at save time, so two
    # exports straddling a second boundary differ. Stabilize for E4.
    payload = stabilize_zip(buf.getvalue())
