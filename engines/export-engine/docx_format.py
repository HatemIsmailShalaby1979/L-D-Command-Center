...
from engines.export_engine.zip_stability import stabilize_zip
...
def _normalize_docx_zip(payload: bytes) -> bytes:
    """Remove ZIP member timestamps that python-docx writes at save time.

    Delegates to the shared OOXML stabilizer so DOCX gets the same
    core.xml timestamp pinning as XLSX/PPTX.
    """
    return stabilize_zip(payload)
