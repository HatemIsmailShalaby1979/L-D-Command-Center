...
__all__ = [
    ...
    "GREENHOUSE_DEFAULT_COMPANIES", "LEVER_DEFAULT_COMPANIES",
    "ASHBY_DEFAULT_COMPANIES",
]

# Default company rosters — the contact-center/CX market the career hunt
# targets. Board tokens verified live via HEAD probes on 2026-09-05 (the
# legacy stripe/figma/vercel roster 404s on every board now). Companies
# migrate boards over time, so a dead token is a cue to re-check the
# roster, never a crash: dead sources are skipped with a warning.
GREENHOUSE_DEFAULT_COMPANIES: list[str] = [
    "dialpad", "five9", "nextiva", "vonage", "intercom", "qualtrics",
    "hubspot", "cresta", "observeai", "maestroqa",
]
LEVER_DEFAULT_COMPANIES: list[str] = ["aircall"]
ASHBY_DEFAULT_COMPANIES: list[str] = ["helpscout", "gorgias", "kustomer", "glia"]
...
def search_all(*,
    ...
    Company lists default to the contact-center/CX rosters above; callers
    may pass their own lists or empty lists to skip a source.
    ...
    if greenhouse_companies is None:
        greenhouse_companies = GREENHOUSE_DEFAULT_COMPANIES
    if lever_companies is None:
        lever_companies = LEVER_DEFAULT_COMPANIES
    if ashby_companies is None:
        ashby_companies = ASHBY_DEFAULT_COMPANIES
