...
class TestSources:
    ...
    def test_search_all_defaults_to_contact_center_rosters(self):
        """No company lists passed -> the engine hunts the verified
        contact-center/CX roster (dialpad, five9, helpscout, ...), not
        an empty search."""
        calls = []
        def t(url):
            calls.append(url)
            return {"jobs": []} if "greenhouse" in url else []
        merged = search_all(include_remote=False,
                            transport_factory=lambda: t)
        hunted = " ".join(calls)
        assert "dialpad" in hunted and "five9" in hunted  # greenhouse
        assert "helpscout" in hunted                        # ashby
        assert "aircall" in hunted                           # lever
