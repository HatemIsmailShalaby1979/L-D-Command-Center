...
class TestFullPipelineLive:
    ...
    def test_live_pipeline(self, tmp_path: Path):
        ...
        # Generate with real model. The model id used to be hardcoded to one
        # machine's loaded model, so this test failed everywhere else. Pick a
        # capable model the same way the rest of the live suite does.
        from conftest import select_live_model
        ...
        journey = _gen.generate_journey(
            ...
            client=client,
            num_cards=2,
            model=select_live_model(client.list_models()),
        )
