...
__all__ = ["LanguageLabRenderer", "render_lesson_pack_html",
           "attach_pack_footer", "PackFooter", "APP_NAME", "APP_URL"]
...
.ldcc-footer{margin-top:2.5rem;padding-top:1rem;border-top:1px solid var(--border);
  color:var(--muted);font-size:.85rem;max-width:720px}
.ldcc-footer a{color:var(--accent);text-decoration:none}
.ldcc-share{display:flex;gap:.6rem;align-items:center;flex-wrap:wrap;
  margin-bottom:.75rem}
.ldcc-share button{padding:.45rem .9rem;border:1px solid var(--accent);
  border-radius:6px;background:var(--accent-light);color:var(--accent);
  cursor:pointer;font-size:.9rem}
.ldcc-sponsor{margin-top:.35rem}
...
function packFileName(){
  var t=(document.title||'lesson-pack').toLowerCase()
    .replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
  return (t||'lesson-pack')+'.html';
}
function packMarkup(){
  return '<!DOCTYPE html>\\n'+document.documentElement.outerHTML;
}
function sharePack(){
  var name=packFileName();
  try{
    var file=new File([new Blob([packMarkup()],{type:'text/html'})],name,
                      {type:'text/html'});
    if(navigator.canShare&&navigator.canShare({files:[file]})){
      navigator.share({files:[file],title:document.title,
        text:'Language lesson pack — made offline with L&D Command Center'}
        ).catch(function(){ downloadPack(); });
      return;
    }
  }catch(e){}
  downloadPack();
...
function downloadPack(){
  ...
  a.href=URL.createObjectURL(new Blob([packMarkup()],{type:'text/html'}));
...
# ---------------------------------------------------------------------------
# Shareable footer (PROFIT_PLAN §3.1 + §4.5)
#
# Every exported pack is a landing page: it carries the app's name and a
# one-click "share / save" button that writes the page (audio included,
# as embedded data URIs) back out as a single file. Free users share for
# free — the growth loop is never gated. A school or tutor may brand the
# footer (optional sponsored channels); the product placements are
# visible text, never tracking and never data collection.
# ---------------------------------------------------------------------------

APP_NAME = "L&D Command Center"
APP_URL = "https://github.com/HatemIsmailShalaby1979/L-D-Command-Center"

_FOOTER = """\
<footer class="ldcc-footer">
  <div class="ldcc-share">
    <button onclick="sharePack()">Share / save this pack</button>
    <span>One self-contained HTML file — audio included. Send it, post it,
      teach with it.</span>
  </div>
  <p>{tagline}</p>
{sponsor}</footer>
...
_DEFAULT_TAGLINE = (
    'Made with <a href="{url}">{app}</a> — free, offline, runs on your own '
    'laptop. No account, no cloud: your learning history never leaves your '
    'machine.'
).format(url=html.escape(APP_URL, quote=True), app=html.escape(APP_NAME))

_SPONSOR_LINE = '<p class="ldcc-sponsor">{brand}</p>\n'


@dataclass(frozen=True)
class PackFooter:
    """Branding carried by an exported pack (defaults = the app only)."""
    brand: str = ""
    brand_url: str = ""
    note: str = ""

    @classmethod
    def from_dict(cls, data: Optional[dict[str, Any]]) -> "PackFooter":
        data = data or {}
        return cls(brand=str(data.get("brand") or ""),
                   brand_url=str(data.get("brand_url") or ""),
                   note=str(data.get("note") or ""))

    @staticmethod
    def _safe_url(raw: str) -> str:
        """Branding links are http(s) only. A `javascript:` URL in a
        shared pack would execute in the recipient's browser — the one
        place a user-supplied string becomes a link, so it is allowlisted
    ...
    def render(self) -> str:
        sponsor = ""
        if self.brand.strip():
            name = html.escape(self.brand.strip())
            url = self._safe_url(self.brand_url)
            if url:
                name = f'<a href="{url}">{name}</a>'
            line = f"Pack published by {name}."
            if self.note.strip():
                line += f" {html.escape(self.note.strip())}"
            sponsor = _SPONSOR_LINE.format(brand=line)
        return _FOOTER.format(tagline=_DEFAULT_TAGLINE, sponsor=sponsor)
...
class LanguageLabRenderer:
    ...
    def render(self, pack: dict[str, Any], *,
               ...
               footer: Optional[PackFooter] = None) -> str:
        ...
        head = (f"<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n"
                ...
                f"</head>\n<body>\n")
        tail = (_FINAL_SCREEN + "\n<script>" + _JS + "</script>\n</body>\n</html>\n")
        # The footer is part of the shareable artifact, not decoration:
        # every pack that leaves the machine advertises where it came
        # from (PROFIT_PLAN §3.1). It is deterministic — no timestamps.
        footer_html = (footer or PackFooter()).render()
        out = (head + "\n".join(parts) + "\n" + _FINISH_BAR + "\n"
               + footer_html + "\n" + tail)
...
def render_lesson_pack_html(
    pack: dict[str, Any], *,
    audio_files: Optional[dict[str, str]] = None,
    footer: Optional[PackFooter] = None,
) -> str:
...
def attach_pack_footer(document: str,
                       footer: Optional[PackFooter] = None) -> str:
    """Add the shareable footer to an ALREADY-RENDERED pack, in place.

    Re-sharing reuses the stored HTML instead of re-rendering from JSON
    because the rendered file carries the dialogue audio as embedded data
    URIs; re-rendering from the pack JSON would silently strip it. Safe
    to call twice: a document that already has a footer is returned
    unchanged, and a document from an older build that lacks the share
    handler gets the script injected too.
    """
    if 'class="ldcc-footer"' in document:
        return document
    markup = (footer or PackFooter()).render()
    if "function sharePack" not in document:
        markup = "<script>" + _JS + "</script>\n" + markup
    if "</body>" in document:
        return document.replace("</body>", markup + "</body>", 1)
