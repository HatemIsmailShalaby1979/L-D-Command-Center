...
from engines.language_lab.renderer import PackFooter, render_lesson_pack_html
...
class TestShareableFooter:
    """PROFIT_PLAN §3.1 (growth loop) + §4.5 (sponsored pack channels)."""

    def test_every_pack_carries_the_app_credit(self, html):
        assert "L&amp;D Command Center" in html
        assert "offline" in html
        assert 'class="ldcc-footer"' in html

    def test_share_button_and_download_js_present(self, html):
        assert "sharePack()" in html
        assert "downloadPack()" in html
        assert "packFileName()" in html
        # the saved file is a full document, not a fragment
        assert "document.documentElement.outerHTML" in html
    ...
    def test_footer_accepts_sponsor_branding(self):
        footer = PackFooter(brand="Berlin Language Lab",
                            brand_url="https://example.edu",
                            note="Sponsored pack.")
        html = render_lesson_pack_html(good_pack(), footer=footer)
        assert "Berlin Language Lab" in html
        assert 'href="https://example.edu"' in html
        assert "Sponsored pack." in html

    def test_branding_cannot_inject_markup(self):
        footer = PackFooter(brand='<script>evil()</script>',
    ...
    def test_non_http_brand_url_is_dropped_not_linked(self):
        footer = PackFooter(brand="Lab", brand_url="ftp://example.edu")
        html = render_lesson_pack_html(good_pack(), footer=footer)
        assert "Pack published by Lab." in html
        assert "ftp://" not in html

    def test_footer_from_dict_defaults_to_unbranded(self):
        assert PackFooter.from_dict(None) == PackFooter()
        assert PackFooter.from_dict({"brand": "A"}).brand == "A"
