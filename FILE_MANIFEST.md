...
| `/engines/audio-engine/podcast_script.py` | Podcast script generation — topic/Journey → structured script with schema validation, length-compliance word budget (TARGET_SPEAKING_WPM/MIN_CONTENT_WPM/make_length_validator), and retry logic |
| `/engines/audio-engine/test_podcast_script.py` | ~40 tests: dataclass validation, schema validation, length validator (word budget floor), generation, retry logic, error cases, convenience functions |
...
| `/engines/language-lab/immersion.py` | Immersion podcast generation — topic+target language → PodcastScript in target language via audio-engine, rendered with two distinct target-language voices; length stretch re-render when audio <92% of requested minutes (floor 0.70×) |
| `/engines/language-lab/test_immersion.py` | 19 tests: ImmersionResult properties, script generation delegation, audio rendering delegation, parameter passing, length stretch re-render, error handling |
