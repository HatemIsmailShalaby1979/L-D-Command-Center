...
| `/desktop-shell/app.py` | Thin Tkinter window over the controller; tkinter imported only at runtime (P5.2); P7.12 Playground tab — canvas list (library/generated/inbox), file-picker import + inbox scan (native tkinter has no OS drop target; inbox folder is the drop path), connector panel rendering quota notes + prompt Generate; P8.8 loading indicators (button disabled + status text), P8.9 user-friendly errors with ERROR_ACTIONS mapping |
