...
class PromptRegistry:
    ...
    _PODCAST_USER_BASE = (
        ...
        "- Each segment's 'content' MUST contain approximately "
        "{segment_words} words of spoken text (intro/conclusion may run "
        "a little shorter). The TOTAL words across all segments must be "
        "at least {total_words} words — the audio length is determined "
        "by word count, not by the duration_seconds field. Thin content "
        "produces a podcast far shorter than {duration_minutes} minutes.\n"
    ...
    _PODCAST_RETRY_USER_TEMPLATE = (
        ...
        "Each segment's content must be about {segment_words} words; "
        "total across all segments at least {total_words} words — audio "
        "length follows WORD COUNT, so thin content will fail again.\n\n"
