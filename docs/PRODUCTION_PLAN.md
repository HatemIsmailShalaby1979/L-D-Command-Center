...
| E5 installable offline desktop build | ✅ | Linux ✅ 2026-08-25 (dist/ldcc onefile, smoke-run clean vs live LM Studio); Windows ✅ 2026-09-13 (dist/ldcc.exe rebuilt + verified: window renders, no error dialog — crash root cause: frozen app clobbered packaged packages with source aliases in app.py, fixed via `sys.frozen` gate) |
...
Blocking v1 tag: none — E5 Windows packaging verified on a real OS target.
